...
//
//       Ajout Mqtt en lieu et polace de d'Emoncms car Emoncms accepte MQTT
//       Publication IP et date/heure en + de DATA Json
//
// **********************************************************************************
...
//Client NTP
WiFiUDP ntpUDP;
//NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", 3600, 60000); // 3600=fuseau 60000=periode mah = 1h
NTPClient timeClient(ntpUDP, config.ntp_host, 3600, 60000); // 3600=fuseau 60000=periode mah = 1h

RTC_Millis rtc;
unsigned long DateHeure;
DateTime DTHH;
String StrDThh;
...
void ResetConfig(void) 
{
  ...
  strcpy_P(config.ntp_host, CFG_NTP_DEFAULT_HOST);
...
boolean adjustDstEurope()
{
  ...
  int beginDSTDate= (31 - (5* DTHH.year() /4 + 4) % 7);
  int beginDSTMonth=3;
  //last sunday of october
  int endDSTDate= (31 - (5 * DTHH.year() /4 + 1) % 7);
  int endDSTMonth=10;
  // DST is valid as:
  if (((DTHH.month() > beginDSTMonth) && (DTHH.month() < endDSTMonth))
    || ((DTHH.month() == beginDSTMonth) && (DTHH.day() >= beginDSTDate))
    || ((DTHH.month() == endDSTMonth) && (DTHH.day() < endDSTDate)))
...
//    DebugF("Connexion au serveur MQTT...");
    MQTTclient.connect(config.mqtt.topic, config.mqtt.user, config.mqtt.pswd);
//    if (MQTTclient.connect(config.mqtt.topic, config.mqtt.user, config.mqtt.pswd)) {
        //DebuglnF("OK");
//     } 
//     else {
//      DebugF("KO, erreur : ");
//      //Debug(MQTTclient.state());
//      DebuglnF(" On attend période mqtt suivante avant de recommencer");
//    }
}
...
//  Debugflush();
  ...
  server.on("/mqtt.json", mqttJSONTable);
...
void loop()
{
  ...
  if (task_1_sec) { 
    ...
    if ( WiFi.status() == WL_CONNECTED){
        // Gestion horloge
        timeClient.update();
        DateHeure=timeClient.getEpochTime();
        //Check summer / winter time
        if (adjustDstEurope()) {
         // Debugln("heure ete");
          DateHeure = DateHeure + 3600;
        }
        rtc.adjust(DateHeure);
        DTHH= rtc.now();
        StrDThh=String(DTHH.year(),DEC) +'/'+ String(DTHH.month(),DEC) +'/'+ String(DTHH.day(),DEC) +' '+ String(DTHH.hour(),DEC)+':'+ String(DTHH.minute(),DEC)+':'+ String(DTHH.second(),DEC);
    //    Debugln(StrDThh);
