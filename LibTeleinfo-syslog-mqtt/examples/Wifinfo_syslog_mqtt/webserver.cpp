...
void handleFormConfig(void) 
{
  ...
  if (server.hasArg("save"))
  {
    ...
    strncpy(config.ntp_host ,   server.arg("ntp_host").c_str(),     64 );
...
/* ======================================================================
Function: mqttJSONTable (added by Doume)
Purpose : prepare the JSON table needed to fill mqtt server with values
            some values have been translated, because mqtt only
...
void mqttJSONTable()
{
  Debug(F("Serving /mqtt.json page..."));
  String response = build_mqtt_json(); 
...
void getConfJSONData(String & r)
{
  ...
  r+=CFG_FORM_NTP_HOST; r+=FPSTR(FP_QCQ); r+=config.ntp_host;  r+= FPSTR(FP_QCNL); 
